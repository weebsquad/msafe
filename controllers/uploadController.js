...
uploadsController.fileInfo = async (req, res, next) => {
	...
	let _usrUpl = await db.table('users').where('id', file.userid).first();
	...
	if (typeof (file.timestampExpire) === 'number') _fileInfo['Timestamp Expire'] = utils.getPrettyDate(new Date(file.timestampExpire));
	...
	if (utils.isAdmin(user.username)) {
		...
		if (typeof (file.timestamp) === 'number') _fileInfo['Upload Date'] = utils.getPrettyDate(new Date(file.timestamp*1000));
