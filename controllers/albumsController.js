...
albumsController.generateZip = async (req, res, next) => {
	if (config.s3.use && config.uploads.generateZips) return res.status(401).json({ success: false, description: 'Temporarily disabled'});
	const identifier = req.params.identifier;
	if (identifier === undefined) return res.status(401).json({ success: false, description: 'No identifier provided' });
	...
	if (album.zipGeneratedAt > album.editedAt) {
		...
		for (let file of files) {
			try {
				fs.statSync(path.join(__dirname, '..', config.uploads.folder, file.name));
